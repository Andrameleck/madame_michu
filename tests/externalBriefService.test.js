...
test("compose un bulletin meteo et actualites avec les API publiques", async () => {
  ...
  const context = loadService(async (url) => {
    ...
    if (String(url).includes("open-meteo.com/v1/forecast")) {
      return {
        ok: true,
        json: async () => ({
          daily: { time: ["2026-08-21"], weather_code: [61], temperature_2m_min: [15], temperature_2m_max: [24], precipitation_probability_max: [70], wind_speed_10m_max: [20] },
          current: { time: "2026-08-21T10:00", temperature_2m: 18, weather_code: 61, pressure_msl: 1012 },
          hourly: {
            time: ["2026-08-21T10:00", "2026-08-21T15:00"],
            temperature_2m: [18, 21],
            weather_code: [61, 2],
          },
        }),
      };
  ...
  assert.equal(brief.weather.current.temperature, 18);
  assert.equal(brief.weather.current.pressure, 1012);
  assert.equal(brief.weather.current.icon, "🌧️");
  assert.equal(brief.weather.trend.condition, "ciel partiellement nuageux");
  assert.equal(brief.news[0].title, "Eau et agriculture");
  assert.equal(calls.length, 3);
  assert.ok(calls.some((url) => /timespan=1d/.test(url)));

  context.brief = brief;
  const overview = vm.runInContext("formatExternalNewsOverview(brief, 'fr')", context);
  assert.match(overview, /A la une/);
  assert.match(overview, /Eau et agriculture/);
  assert.doesNotMatch(overview, /pluie/);
});
