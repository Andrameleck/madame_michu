...
recus et detecte automatiquement les propositions de rendez-vous pour les
ajouter au calendrier (Lightning), apres validation manuelle.
...
llm/
  promptBuilder.js      construction du prompt (system + user)
  ollamaClient.js        appel HTTP vers l'API Ollama (/api/chat)
  responseParser.js     parsing robuste du JSON retourne par le LLM
calendar/
...
ui/
  sidebar/                resume + liste des RDV a valider (ouvert via le bouton de la barre d'outils)
